...
if initialize == "manual":
	...
	rng=np.random.default_rng(9837123)
	k = (kb - ka) * np.random.rand(4) + ka
	req = (reqb - reqa) * np.random.rand(4) + reqa
	xp = (xpb - xpa) * np.random.rand(4) + xpa
	yp = (ypb - ypa) * np.random.rand(4) + ypa
	ro = (rob - roa) * rng.random() + roa
	vo = (vob - voa) * rng.random() + voa
	thetao = (thetaob - thetaoa) * rng.random() + thetaoa
	omegao = (omegaob - omegaoa) * rng.random() + omegaoa 
...
xmax = max(x) if max(x) > max(xp) else max(xp)
xmin = min(x) if min(x) < min(xp) else min(xp)
ymax = max(y) if max(y) > max(yp) else max(yp)
ymin = min(y) if min(y) < min(yp) else min(yp)

if initialize == "automatic":
	mrf = 1/50
